    ...
    def do_long(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(x_times) > 0:
            ...
            if len(chunk_times) > 0:
                ...
                if dist_xy > 5 and diff_close_xy < 0.2 and x_close > y_close: # level
                # if dist_xy > 1 and diff_close_xy < 0.1:
    ...
    def do_short(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(x_times) > 0:
            ...
            if len(chunk_times) > 0:
                ...
                # if dist_xy > 1 and diff_close_xy < 0.8 and x_close < y_close: <------ alternative
                if dist_xy > 5 and 0 < diff_close_xy < 0.2 and x_close < y_close: # level
                # if dist_xy > 1 and 0 < diff_close_xy < 0.1:
                    for i, row in df.iterrows():
                        dist_xi = i - x
